...
static long int zhBase64Encode( char *src,long int src_len, char *dst)
{
    long int i = 0, j = 0;
    //  printf("%ld\n",src[2]);
...
static int _zhBase64Decode( unsigned char *input, unsigned int input_len, unsigned char *output, unsigned int *output_len )
{	
...
static int zhBase64Decode(unsigned char *in, unsigned int inLength, unsigned char **out)
{
	int len;
	unsigned int outLength = 0;
	
	//should be enough to store 6-bit buffers in 8-bit buffers
	len=inLength * 3.0f / 4.0f + 1;
	*out = malloc(len);
	(*out)[len-1]=0;
	if( *out ) {
